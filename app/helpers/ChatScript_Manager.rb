...
class ChatScriptManager
  ...
  def manage
    ...
    #have to figure out how to split text in adequate group of sentences
    raw_reply = volley.text
    reply = raw_reply.scan(/[^\.!?]+[\.!?]/).map(&:strip)
    ...
    if @message==':build mine'
        @api.call('sendMessage', chat_id: @user.telegram_id, text: raw_reply)
    else
=begin
        if answers.nil?
          @api.call('sendMessage', chat_id: @user.telegram_id, text: raw_reply)
        else
          @api.call('sendMessage', chat_id: @user.telegram_id, text: raw_reply, reply_markup: answers)
        end
=end
  ...
  def process_oob(user_state_oob)
    ...
    if user_state_oob!=''
      ...
      ################## ERROR ON BUTTONS - NOT FOUND METHOD #########################
      if JSON.parse(user_state_oob).key?("buttons")
